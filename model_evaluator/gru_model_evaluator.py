...
early_stop = keras.callbacks.EarlyStopping(monitor='loss', min_delta=0.0001, patience=1, mode='min')

def evaluate_gru_model():

    # Load saved model.
    model = keras.models.load_model('hyper_model/best_model/best_gru_model.keras')
    ...
    print('Evaluation of gru model:\n')
    print(f'Test Metrics MSE, MAE, MAPE, and R2: {[round(elem, 6) for elem in test_result]}')
    # print(
    ...
    for train_index, test_index in tscv.split(X):
        ...
        # Store the result
        fold_results['mse'].append(round(metrics[0], 6))
        fold_results['mae'].append(round(metrics[1], 6))
        fold_results['mape'].append(round(metrics[2], 6))
        fold_results['r2_score'].append(round(metrics[3], 6))
    ...
    fig.suptitle(f'{k_fold}-Fold Time Series Validation of GRU-Dense Model')
...
def plot_gru_model_prediction():
    ...
    plt.xlabel('Days')
    plt.ylabel('Close')
    plt.title('NEPSE Index Prediction with GRU-Dense Model')
    plt.grid(True)
    plt.legend(loc='best')
    plt.show()

def test_plot_gru_model_prediction(unseen_data=None):
...
def plot_gru_training_history():

    # Plot Learning Curves
    
    history = pd.read_csv('hyper_model/best_model/best_gru_model_history.csv')

    fig, (ax1, ax2) = plt.subplots(2, sharex=True)
    fig.suptitle('Trainig History of GRU-Dense Model')
...
# plot_gru_model_prediction()
test_plot_gru_model_prediction(unseen_data)
plot_gru_training_history()
evaluate_gru_model()
