...
from variable_config import LOOK_BACK as look_back, EPOCHS, MAX_EPOCHS, file_name
from data_wrangler import data_wrangler, split_into_datasets, train_val_test_split
...
def train_model():
    ...
    check_point = keras.callbacks.ModelCheckpoint(
        # 'model_saved/check_point/{epoch:03d}-{val_loss:.4f}.keras',
        'model_saved/check_point/epoch_{epoch:03d}.keras',
        monitor='val_loss',
        mode='min',
        save_best_only=True,
        save_freq='epoch'
    ...
    hb_tuner.search(X_train, y_train, validation_data=(X_val, y_val), callbacks=[early_stop, check_point])
    ...
    history = model.fit(X_train, y_train, validation_data=(X_val, y_val), callbacks=[early_stop, check_point])
    ...
    model.save('model_saved/best_model.keras')
    # best_model.save('model_saved/best_model.h5')
    # Saves the entire model as a SavedModel. It places the contents of model in a directory.
    # best_model.export('model_saved/saved_model/best_model')
